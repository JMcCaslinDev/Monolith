...
final class StickiesServiceTest extends TestCase
{
    ...
    /** Users only see their own stickies on the board. */
    public function test_list_notes_isolates_by_user(): void
    {
        $svc = new StickyService($this->db);
        $svc->saveNote(1, 'Mine', 'general', 'yellow', 'board');
        $otherId = $this->insertMember();
        $svc->saveNote($otherId, 'Theirs', 'work', 'pink', 'board');

        $this->assertCount(1, $svc->listNotes(1));
        $this->assertSame('Mine', $svc->listNotes(1)[0]['content']);
    }

    /** Cross-user updates are rejected as not found. */
    public function test_cannot_update_another_users_note(): void
    {
        $svc = new StickyService($this->db);
        $note = $svc->saveNote(1, 'Private', 'general', 'yellow', 'board');
        $otherId = $this->insertMember();

        $this->expectException(\RuntimeException::class);
        $svc->saveNote($otherId, 'Hijacked', 'general', 'yellow', 'board', (int) $note['id']);
    }

    /** Blank category and section labels fall back to board defaults. */
    public function test_empty_labels_normalize_to_defaults(): void
    {
        $svc = new StickyService($this->db);
        $note = $svc->saveNote(1, 'Defaults', '  ', 'yellow', '   ');

        $this->assertSame('general', $note['category']);
        $this->assertSame('board', $note['section']);
    }

    /** Deleting a missing sticky throws so the UI can show an error. */
    public function test_delete_missing_note_throws(): void
    {
        $svc = new StickyService($this->db);
        $this->expectException(\RuntimeException::class);
        $svc->deleteNote(1, 99999);
    }

    /** Empty search returns all notes without filtering. */
    public function test_filter_by_search_empty_query_returns_all(): void
    {
        $rows = [
            ['content' => 'A', 'category' => 'general', 'section' => 'board'],
            ['content' => 'B', 'category' => 'work', 'section' => 'ideas'],
        ];
        $this->assertCount(2, StickyService::filterBySearch($rows, '   '));
    }

    /** Move without a new section keeps the sticky on its current section. */
    public function test_move_without_section_keeps_section(): void
    {
        $svc = new StickyService($this->db);
        $note = $svc->saveNote(1, 'Stay', 'general', 'yellow', 'ideas');
        $moved = $svc->moveNote(1, (int) $note['id'], 10, 20);

        $this->assertSame('ideas', $moved['section']);
    }
}
